...
/*
  FleetOS — dashboard navbar
  ------------------------------------------------------------------
  Same design tokens as the rest of FleetOS (see Home.jsx for the full
  notes) — this is the top bar for logged-in dashboard pages (Vehicles,
  Orders, ...), as opposed to the marketing Nav on the homepage.

  This file assumes the host page already loads the three Google Fonts
  and defines the `fleetos-display` / `fleetos-body` / `fleetos-mono`
  classes (see the <style> block in Vehicles.jsx) — it doesn't repeat
  that import here since it's meant to sit inside a page that already
  has it.

  This is the navbar for regular users — no create/delete actions live
  here. Those belong to a separate admin-only page.

  Usage:
    <Navbar active="vehicles" userName="Shiva Kumar" />
*/
...
function Avatar({ initials }) {
  return (
    <span className="flex h-8 w-8 flex-none items-center justify-center rounded-full bg-[#0B0E14] fleetos-mono text-[12px] font-semibold text-white">
      {initials}
    </span>
  )
}
      ...
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-6">
        <div className="flex items-center gap-8">
          <a href="#top" className="flex items-center gap-2.5">
            ...
            <span className="fleetos-display text-[17px] font-semibold tracking-tight text-[#0B0E14]">
              FleetOS
            </span>
          </a>
          ...
          <nav className="hidden items-center gap-1 md:flex">
            {links.map((l) => (
              <a
                ...
                className={
                  l.key === active
                    ? 'rounded-full bg-[#F3F5F8] px-4 py-2 fleetos-body text-[13.5px] font-semibold text-[#0B0E14]'
                    : 'rounded-full px-4 py-2 fleetos-body text-[13.5px] font-medium text-[#5B6472] transition-colors hover:text-[#0B0E14]'
                }
              >
                {l.label}
              </a>
        ...
        <div className="hidden items-center gap-3 md:flex">
          ...
          <div className="relative">
            ...
            >
              ...
              <span className="fleetos-body text-[13px] font-medium text-[#0B0E14]">{userName}</span>
            ...
            {profileOpen && (
              <>
                ...
                <div className="absolute right-0 z-20 mt-2 w-48 rounded-xl border border-slate-200 bg-white p-1.5 shadow-[0_12px_24px_-8px_rgba(15,23,42,0.18)]">
                  <a
                    href="#"
                    className="flex items-center gap-2.5 rounded-lg px-3 py-2 fleetos-body text-[13.5px] text-[#0B0E14] hover:bg-[#F3F5F8]"
                  >
        ...
        <div className="flex flex-col gap-1 border-t border-slate-200 bg-white px-6 py-4 md:hidden">
          {links.map((l) => (
            <a
              ...
              className={
                l.key === active
                  ? 'rounded-lg bg-[#F3F5F8] px-3 py-2.5 fleetos-body text-sm font-semibold text-[#0B0E14]'
                  : 'rounded-lg px-3 py-2.5 fleetos-body text-sm text-[#0B0E14]'
              }
            >
              {l.label}
            </a>
          ...
          <div className="mt-3 flex items-center gap-2.5 border-t border-slate-100 pt-3">
            <Avatar initials={getInitials(userName)} />
            <span className="fleetos-body text-[13.5px] font-medium text-[#0B0E14]">{userName}</span>
          </div>
        </div>
      )}
    </header>
